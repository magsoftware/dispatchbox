...
from dispatchbox.worker import OutboxWorker, HandlerNotFoundError
...
def test_run_loop_skips_event_without_id(mock_repository):
    ...
    mock_repository.fetch_pending.return_value = [event_no_id]
    ...
    with patch("dispatchbox.worker.time.sleep"):
        worker.run_loop()
